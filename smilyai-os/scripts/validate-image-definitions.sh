...
cd "$PROJECT_DIR"
while IFS= read -r file; do bash -n "$file"; done < <(find scripts packaging -type f \( -name '*.sh' -o -name '*.hook.chroot' -o -name 'autostart' -o -name 'smilyai-session' -o -name 'smilyai-shell' \))
