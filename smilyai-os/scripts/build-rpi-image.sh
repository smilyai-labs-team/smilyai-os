...
CREDENTIALS="$OUTPUT_DIR/SmilyAI-OS-0.3.0-rpi-arm64.credentials.txt"
...
shopt -s nullglob
images=(deploy/*SmilyAI-OS-0.3.0*.img.xz)
[[ "${#images[@]}" == 1 ]] || { echo "Expected exactly one custom-stage image; inspect deploy/."; exit 1; }
xz -t "${images[0]}"
IMAGE_TARGET="$OUTPUT_DIR/SmilyAI-OS-0.3.0-rpi-arm64.img.xz"
