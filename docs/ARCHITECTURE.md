...
| --- | --- | --- |
| smilyai-ui | Static assets and same-origin API proxy on loopback 47810 | Restart after one second |
...
| smilyai-shell | Sandboxed Chromium application window | Restart after failure; native shortcuts remain |

No network-online dependency, AI call, shader compile or microphone grant blocks
the session. The launcher waits at most approximately four seconds for static assets,
then exits for systemd to retry. Native labwc shortcuts work independently of both
HTTP processes: Super+Return terminal, Super+E files, Super+N network,
Super+R restart the shell services, Super+Space raise SmilyAI, Alt+Tab switch apps.
