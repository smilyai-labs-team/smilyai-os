...
sha256sum -c dist/SmilyAI-OS-0.3.0-x86_64.iso.sha256
```

Expected output: `dist/SmilyAI-OS-0.3.0-x86_64.iso`, checksum, runtime manifest and
...
sha256sum -c dist/SmilyAI-OS-0.3.0-rpi-arm64.img.xz.sha256
xz -t dist/SmilyAI-OS-0.3.0-rpi-arm64.img.xz
```
...
Read the owner-only `dist/SmilyAI-OS-0.3.0-rpi-arm64.credentials.txt` locally.
...
qemu-system-x86_64 -enable-kvm -cpu host -m 4096 -smp 4 \
  ...
  -cdrom dist/SmilyAI-OS-0.3.0-x86_64.iso -boot d \
